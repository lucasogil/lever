...
<ingresso:template>
    <jsp:body>
        ...
        <form action='/admin/sessao' method="post">
            ...
            <div class="form-group">
                <label for="curso">Filme:</label>
                <select id="curso" name="filmeId" class="form-control">
                    <option value="">Selecione um Filme</option>
                    <c:forEach var="curso" items="${filmes}">
                        <option value="${curso.id}" ${curso.id.equals(form.filmeId)? "selected": ""}>${curso.nome}</option>
                    </c:forEach>
                </select>
